# Playback node — receive-only, master-driven audio receiver

This is a **self-contained** implementer's spec for an ensemble **playback node**:
a receive-only participant (a Go playback-only daemon, or ESP32-S3 firmware) that
plays a group's audio in sync. You do not need to read any other document to
implement it. The reference implementation is
[`cmd/dumbclient/main.go`](../cmd/dumbclient/main.go) (pure stdlib, no `internal/`
imports — it proves this spec is sufficient).

> **"Dumb client" = the minimal MCU profile of the playback role.** Per
> [arch decisions D49–D56](arch/DECISIONS.md), the binary has two
> independently-enableable roles — **`master`** (gossips, owns cluster state,
> serves the API + SPA, sources audio) and **`playback`** (this document). The
> playback protocol is **identical for Go and MCU**; they differ only in announced
> **capabilities** (§6). A combined node runs both roles.

---

## 1. Scope — what a playback node is and is NOT

A playback node:

- **announces itself over mDNS** (§6) so masters can discover it — but it
  **never gossips** and holds **no cluster state**;
- is **driven by a master** over a small control plane (§7): a master tells it to
...
- is **invisible to codec negotiation**: it does not appear in the gossiped
  ...
  playback node whose announced codecs (§6) don't cover the group codec, but the
  codec is not changed for it.

**This is the v2 model.** It replaces the old self-directed dumb client that polled
`GET /api/cluster` and chose its own master. That mode survives only as a
**fixed/standalone fallback** (§11) for installs without a master driving them.

Because you are invisible to negotiation, **you cannot rely on the group running a
codec you support.** The default group codec is **opus**. A protocol-minimal client
typically wants **PCM** (no decoder) — so either the group is pinned to `pcm`, or
you implement an **opus** decoder (libopus). The `ATTACH` (§7) tells you which codec
...
> v2 adds the control-plane types (`0x30`–`0x40`, §7) and the playback node's mDNS
> announcement (§6) to the v1 data plane. The data-plane wire (header, audio, FEC,
...
(`0x2x`/`0x3x`/`0x40`) `gen`/`seq`/`pts` are unused unless stated; set them to 0.

### Packet types

**Data plane** (master's SOURCE_PORT / STREAM_PORT — unchanged from v1):

| Type   | Name        | Direction      | Playback node | Notes                                  |
|--------|-------------|----------------|---------------|----------------------------------------|
...
**Control plane** (the playback node's **CONTROL_PORT**, and STATUS back to the
master's SOURCE_PORT — **new in v2**):

| Type   | Name      | Direction        | Playback node | Notes                                          |
|--------|-----------|------------------|---------------|------------------------------------------------|
| `0x30` | ATTACH    | master → pb      | **required**  | "join this stream"; payload §7.1               |
| `0x31` | DETACH    | master → pb      | **required**  | "leave, go idle"; no payload                   |
| `0x32` | SETVOL    | master → pb      | recommended   | volume + mute; payload §7.2                     |
| `0x33` | SETDELAY  | master → pb      | recommended   | output-delay (ms, signed); payload §7.2         |
| `0x34` | SETCAP    | master → pb      | optional      | enable/disable a capability; payload §7.2       |
| `0x35` | SETEQ     | master → pb      | optional      | cross-room equalization delay (ms, unsigned, added); payload §7.2 (D65) |
| `0x40` | STATUS    | pb → master      | recommended   | telemetry; payload §7.3                         |

**Mandatory:** AUDIO, CLOCK_REQ/RSP, HELLO, RECONFIG, ATTACH, DETACH.
**Recommended:** RESTART, SETVOL, SETDELAY, STATUS.
...
A playback node binds **two** sockets:
...
A playback node advertises the standard `_ensemble._tcp` service (domain `local.`)
with a TXT record. Masters browse this service to discover playback nodes. A
playback-only node advertises **no gossip port** (it never joins memberlist).
...
| `control` | `9300`                   | CONTROL_PORT (master→playback commands)                   |
...
A master advertises `role=master` plus the existing `gossip`/`http`/`stream`/
`source` ports. A combined node advertises both sets. **Capabilities never gate
membership or fan-out** — they inform the master/operator (e.g. a UI warning), not
the codec choice.

Typical MCU advert: `id=…  role=playback  control=9300  codecs=opus  rate=48000
hwvol=0  delayms=25  queue=1  input=0`.

---

## 6. Control plane — being driven by a master

A playback node is a **soft-state** receiver: the master periodically re-asserts the
...
### 6.2 DETACH / SETVOL / SETDELAY / SETCAP / SETEQ (master → playback)
...
- **SETEQ (`0x35`)** — 2 bytes: `delayMs` (u16, unsigned). Cross-room equalization
  delay the master computes (D65): it ADDS this much delay so your room — if it has a
...
### 6.3 STATUS (`0x40`, playback → master)

Send to the master's `sourceEndpoint` ~1 Hz and on state change. Lets the master see
a starving/drifting room and feed the SPA's per-room health view. The master never
depends on it for correctness — it is observability + adaptation, not control.
...
| 16     | 1    | `flags`        | bit0 synced, bit1 playing, bit2 calibrated (D65)|
...
| 71     | 8    | `deviceDelayNs`| measured output (device) latency, ns (D63)    |
| 79     | 8    | `phaseErrNs`   | playout phase error vs the smoothed model, ns (D64) |

`deviceDelayNs − phaseErrNs` is your servo's calibrated device-queue depth — the
stable per-room constant the master diffs to compute SETEQ (D65). The `calibrated`
...
**Cold-start burst (D53).** To start audio quickly while still withholding playout
...
## 9. Drift correction — the rate servo (Go and MCU)

The playout scheduler above is **master-clock-locked**, so it corrects *when* each
frame starts — but your DAC consumes samples at its own crystal rate (tens of ppm
off). Correcting only the schedule means you periodically skip or pad a whole 20 ms
frame — audible. The canonical fix (and what a full ensemble member does) is a **rate
servo + fractional resampler**:

- the drift signal is your **output-queue depth** (how much audio sits between your
  Write and the speaker — ALSA `snd_pcm_delay`, or your I2S/DMA fill). The scheduler
  is clock-locked, so the DAC's true rate only shows up as the queue slowly filling or
  draining;
- a proportional controller holds the queue at a setpoint by nudging a **resampling
  ratio** (≈ 1 ± tens of ppm), slew-limited so there are no audible rate steps;
...
S3-class FPU handles Catmull-Rom at 48 kHz stereo). **Without it (`queue=0`):** DAC
drift is unobservable → **accept it**; the skip-late/silence-gap behavior of §8 is the
**floor**, not the target. This is what satisfies the inaudible-drift requirement.
...
## 11. Fallback — fixed / self-directed mode (no master driving)

For fixed installs or bring-up without a master driving you, the playback node may
skip the control plane and **self-attach**:
...
- **Self-directed (legacy):** poll `GET http://<master-host>:<httpPort>/api/cluster`
  every 5 s, pick a group (by `--group <id|name>`, else the first with members), find
  the master node, dial-prefer its `observed[*].ip` else its first `addrs` CIDR host,
  and ATTACH yourself to `sourcePort`/`streamPort` with the group's
  `settings.{codec,transport,bufferMs}`.

These bypass mDNS announcement and master control entirely; you lose remote
volume/delay/assignment but the subscribe/clock/playout protocol is identical. Good
for a fixed speaker pointed at a known always-master node, and for the reference
client's test rig.
...
- [ ] **Announces over mDNS** with `role=playback`, its `control` port, and its
      capability TXT keys (§5); **never gossips**.
- [ ] Receives master control on its **CONTROL_PORT**; applies ATTACH / DETACH /
      SETVOL / SETDELAY / SETCAP **idempotently** (§6); is **idle until ATTACHed**.
...
- [ ] (Recommended) Sends **RESTART+prime** after **> 2 s** starvation; emits
      **STATUS** ~1 Hz; runs the **rate servo** when it can observe its output queue.
