...
> **Two roles, one binary.** Per [arch decisions D49–D56](arch/DECISIONS.md), an
> ensemble node runs two independently-enableable roles: a **room** (the `master`
...
| `0x35` | SETEQ     | master → player  | optional      | cross-room equalization delay (ms, unsigned, added); payload §6.2 (D65) |
| `0x40` | STATUS    | player → master  | recommended   | telemetry; payload §6.3                         |
| `0x41` | STATUSREQ | master → player  | recommended   | liveness poll "send STATUS now"; no payload (D60) |
...
non-gossiping player (D60): answering it keeps you alive in the cluster between
