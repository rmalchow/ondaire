...
## Design choices to confirm before Phase 3 (would become D58+)
...
5. **Multi-master convergence** for a discovered playback node reuses the D7 own-record
...
*Done & tested: **Slice A** (cluster represents discovered playback nodes — `NodeRecord`/`NodeView` `PlaybackNode`+`ControlPort`, content-gated `UpsertPlaybackNode`, local-only proxy); **Slice B** (`DeriveGroups` attaches assigned playback nodes, never solo/master; D60 mDNS-freshness liveness; D51 codec-negotiation exclusion in `membersLackingCodec`); **Slice C.1** (STATUS ingest in `source.Server`, `Statuses()`); **C.2** (`playback.remotePlayer` — wire-driving Player); **C.3** (`playback.Driver` — drives assigned playback members of the group it masters: ATTACH+SETVOL+SETDELAY while playing, DETACH idle/unassigned).
...
- `internal/cluster`: `NodeRecord` gains `Role`, `Gossips bool`, `ControlPort`, and the
  ...
  discovered playback peer (D7 reconcile for multi-master convergence). Add the
...
### Phase 4 — clock cold-start burst (D53)
...
### Phase 5 — STATUS telemetry surfaced (D55)
...
- **Output-delay stays behind the existing calibration `<details>`** (`GroupCard.svelte`
  advanced) — calibration sets it automatically (D48); do not add a per-node delay slider
  to the member row.
...
- **Caps inform, never gate** (D51): a pcm-only speaker assigned to an opus group may get
...
- **Player extraction (Phase 1)** is the highest-risk refactor; gate it on the member
  e2e staying green.
- **Liveness/assignment races** across masters — covered by choices #3/#5 (reuse D7).
...
- **Unauthenticated control plane** on the LAN — same trust model as today's gossip/
  source ports; revisit with the D56 auth/HA work, not here.
