...
*(Inter-speaker drift plot — generated into the gitignored `tools/calib/results/` by `tools/calib/lr_drift.py`.)*
...
*(Microphone-vs-servo correlation plot — generated by `tools/calib/compare_drift.py` into `tools/calib/results/`.)*
...
*(Playback-vs-recording-clock drift plot — generated by `tools/calib/` into `tools/calib/results/`.)*
...
- **Servo:** `internal/sink/servo.go` — a low-pass-filtered proportional loop producing a playback-rate correction
  in ppm (clamped, slew-limited). This is the drift correction we want to report.
...
- `internal/api/dto.go`: extend `SinkStatsResp` and its mapper with lowercase-camel
  keys (`ratePPM` precedent → `ratePPMMax`, `ratePPMAvg`, `restarts`,
  `disarms`, `underruns`).
