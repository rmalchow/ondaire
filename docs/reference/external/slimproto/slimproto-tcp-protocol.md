...
## Contents
  ...
  * 1 SlimProto Protocol
  * 2 Client -> Server Communications
    * 2.1 HELO
      * 2.1.1 Capabilities
    * 2.2 "IR " (Note: padded with two spaces to make it up to 4 characters.)
    * 2.3 RESP
    * 2.4 BODY
    * 2.5 META
    * 2.6 STAT
  ...
  * 3 Server -> Client Communication
    * 3.1 Command: "strm"
      * 3.1.1 u, p, a & t commands and replay_gain field
      * 3.1.2 FLAC-specific notes
      * 3.1.3 WMA-specific notes
      * 3.1.4 AAC-specific notes
    * 3.2 Command: "aude"
    * 3.3 Command: "audg"
    * 3.4 Command: "grfb"
    * 3.5 Command: "grfe"
    ...
    * 3.10 Command: "vers"
    * 3.11 Command "visu"
  * 4 Undocumented commands, brief rundown
    * 4.1 Command "audc"
    * 4.2 Command "audp"
    * 4.3 Command "body"
    * 4.4 Command "cont"
...
##  [SlimProto](SlimProto.html "SlimProto") Protocol 

The [SlimProto](SlimProto.html "SlimProto") protocol is the new protocol developed for the squeezebox and replaces the SLIMP3 one. It is designed to allow the players to communicate effectively over [WANs](WANs.html?title=WANs&action=edit&redlink=1 "WANs \(page does not exist\)") as well as [LANs](LANs.html?title=LANs&action=edit&redlink=1 "LANs \(page does not exist\)"). 

This document describes the [SlimProto](SlimProto.html "SlimProto") for Squeezebox firmware versions 20 and above. The current version of the firmware shipped with SqueezeCenter 7.2.1 is 113. This document describes both version imperfectly. 
...
The client also listens on UDP port 3483 for [SlimProto](SlimProto.html "SlimProto") commands from the server it has already established a TCP connection with. It also seems to respond to data sent back in the TCP connection (you send frames back this way). 
...
[SqueezeboxG](SqueezeboxG.html?title=SqueezeboxG&action=edit&redlink=1 "SqueezeboxG \(page does not exist\)") only, draw graphics 
