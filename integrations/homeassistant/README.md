# ensemble — Home Assistant integration

A Home Assistant custom integration that controls [ensemble](../../), the
...
Every ensemble node — a room (`master`) or a player (`playback`, e.g. an ESP32
speaker) — becomes **one `media_player` entity**. Grouping uses HA's
join/unjoin, mapped to ensemble follow/unfollow (the Sonos/HEOS convention).

## What it does

- **Playback**: play / pause / resume / stop / next / seek (seek is gated to
  seekable sources; live streams surface *not seekable* gracefully).
- **Volume**: per-node volume slider; mute is emulated (ensemble has no mute
...
```
<config>/custom_components/ensemble/
```

i.e. copy `custom_components/ensemble` from this folder to
`<config>/custom_components/ensemble`, then restart Home Assistant. On a tagged
release the CI publishes a ready-made `ensemble-hacs.zip` artifact whose paths
start with `custom_components/ensemble/` — unzip it into your `<config>`
directory and restart.

### HACS (optional — for automatic updates)

HACS pulls from a GitHub repository, so use the GitHub mirror
(`github.com/rmalchow/ensemble`) as a **custom repository**:

1. HACS → ⋮ → *Custom repositories* → add the repo URL, category *Integration*.
2. Install **ensemble**, then restart Home Assistant.
...
- **Auto-discovery**: master nodes advertise over mDNS (`_ensemble._tcp`), so HA
...
- **Manual**: *Settings → Devices & services → Add integration → ensemble*, then
...
- Ensemble ≥ the release matching this component's `version`, reachable on the
...
logger:
  logs:
    custom_components.ensemble: debug
```
