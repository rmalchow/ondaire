"""One media_player per ensemble node (room or player)."""
...
async def async_setup_entry(
    ...
    def _discover() -> None:
        ...
        known.update(new)
        if new:
            async_add_entities(EnsembleMediaPlayer(coordinator, nid) for nid in new)
...
class EnsembleMediaPlayer(CoordinatorEntity[EnsembleCoordinator], MediaPlayerEntity):
    """A single ensemble node as an HA media_player."""

    _attr_has_entity_name = False
    _attr_media_content_type = MediaType.MUSIC
    _attr_supported_features = _SUPPORT

    def __init__(self, coordinator: EnsembleCoordinator, node_id: str) -> None:
    ...
    def media_image_remotely_accessible(self) -> bool:
        # Always proxy through HA — never hand the ensemble /cover URL or a remote
    ...
    async def async_mute_volume(self, mute: bool) -> None:
        ...
        if not node:
            return
        if mute:
            # ensemble has no mute field — emulate by stashing + zeroing volume.
    ...
    async def async_play_media(
        ...
        **kwargs: Any,
        ...
        if enqueue in (MediaPlayerEnqueue.ADD, MediaPlayerEnqueue.NEXT):
            # ponytail: ensemble's queue only appends — NEXT is treated as ADD
