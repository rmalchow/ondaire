"""Config flow for ondaire: zeroconf (masters only) + manual host."""
...
class OndaireConfigFlow(ConfigFlow, domain=DOMAIN):
    """Handle a config flow for ondaire."""
    ...
    async def _resolve_cluster_id(self, host: str, port: int) -> str | None:
        ...
        client = OndaireClient(session, f"http://{host}:{port}")
        try:
